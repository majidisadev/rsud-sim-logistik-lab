...
async function ensureOpnameEditable(opnameId: string | number) {
  ...
  if (opnameResult.rows[0].validation_status !== "Belum") {
    return {
      ok: false as const,
      status: 400,
      error: "Cannot modify validated opname",
    };
  }
...
async function buildOpnameWithItemsAndLots(opnameId: string | number) {
  ...
  const items = itemsResult.rows.map((it: any) => ({
    ...
    recorded_temperature:
      it.recorded_temperature ?? it.item_temperature ?? null,
...
router.get("/items", async (req, res) => {
  try {
    ...
    if (
      officerIdRaw !== undefined &&
      officerIdRaw !== null &&
      String(officerIdRaw).trim() !== ""
    ) {
    ...
    res.json({
      data: itemsResult.rows.map((row: any) => ({
        ...row,
        recorded_temperature:
          row.recorded_temperature ?? row.item_temperature ?? null,
  ...
  try {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (!lots) {
          ...
          if (!finalRecordedExpiration) {
            ...
            finalRecordedExpiration =
              expirationResult.rows[0]?.expiration_date || null;
          }
          ...
          await client.query(
            ...
            [
              ...
              isTemperatureMatch(item?.temperature_match)
        ...
        const itemDetail = await client.query(
          "SELECT temperature FROM items WHERE id = $1",
          [itemId],
        );
        ...
        const recordedTemperature =
          item?.recorded_temperature ?? itemDetail.rows[0].temperature ?? null;
        ...
        const blockedLots = await findPendingLotIdsBlockingNewOpname(
          client,
          lotIds as number[],
        );
        ...
        for (const l of lots) {
          ...
          if (recordedExp) {
            if (!minRecordedExpiration || recordedExp < minRecordedExpiration)
              minRecordedExpiration = recordedExp;
          }
    ...
    if (error?.message === "ITEM_HAS_PENDING_LEGACY_OPNAME") {
      return res.status(400).json({
        error:
          "Barang ini masih memiliki stock opname lama yang belum selesai divalidasi",
      });
    }
    if (
      typeof error?.message === "string" &&
      error.message.startsWith("LOTS_PENDING_OPNAME:")
    ) {
      return res.status(400).json({
        error:
          "Salah satu lot masih dalam stock opname yang belum selesai divalidasi",
      });
    }
    res.status(500).json({ error: "Internal server error" });
  }
...
router.patch(
  "/:id/items/:itemId/lots/:itemLotId",
...
router.patch(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      try {
        await client.query("BEGIN");
        await applyOpnameValidationDecisions(
          client,
          id,
          req.body ?? {},
          req.user!.id,
