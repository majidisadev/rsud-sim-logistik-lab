...
export async function applyOpnameValidationDecisions(
  ...
  const soCheck = await client.query(
    `SELECT id, validation_status FROM stock_opnames WHERE id = $1`,
    [stockOpnameId],
  );
  ...
  for (const it of items) {
    ...
    await client.query(
      `UPDATE stock_opname_items SET ${sets.join(", ")} WHERE id = $${n}`,
      vals,
    );
  }
  ...
  for (const lot of lots) {
    ...
    await client.query(
      `UPDATE stock_opname_item_lots SET ${sets.join(", ")} WHERE id = $${n}`,
      vals,
    );
  }
  ...
  for (const row of soilRows.rows) {
    if (
      row.stock_validation_status === "Disetujui" &&
      !row.stock_adjustment_posted
    ) {
      ...
      await client.query(
        `UPDATE stock_opname_item_lots SET stock_adjustment_posted = TRUE WHERE id = $1`,
        [row.id],
      );
    }
    ...
    if (row.expiration_validation_status === "Disetujui") {
      if (datesDiffer(row.recorded_expiration, row.opname_expiration)) {
        await client.query(
          `UPDATE lots SET expiration_date = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
          [normDate(row.opname_expiration), row.lot_id],
        );
      }
    }
  }
  ...
  for (const soi of soiRows.rows) {
    ...
    if (soi.temperature_validation_status === "Disetujui" && hasLots) {
      ...
      if (
        String(recT ?? "") !== String(opT ?? "") &&
        opT !== null &&
        String(opT).trim() !== ""
      ) {
        await client.query(
          `UPDATE items SET temperature = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
          [opT, soi.item_id],
        );
      }
    }
    ...
    if (!hasLots) {
      if (
        soi.stock_validation_status === "Disetujui" &&
        !soi.legacy_stock_adjustment_posted
      ) {
